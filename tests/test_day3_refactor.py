...
class TestInstructionsUX:
    ...
    def test_instructions_under_word_budget(self, family):
        text = _instructions_for(family, "anything")
        # Checklist UX + view-before-submit guard + SEO (site_name / og:url /
        # JSON-LD url) + endpoint correction + optional-section data. Hard
        # ceiling 1100 words after all 3 May-27 parallel branches merged.
        word_count = len(text.split())
        assert word_count <= 1100, f"[{family}] instructions are {word_count} words; ceiling 1100"
